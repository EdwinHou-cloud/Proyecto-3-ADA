# Importar los módulos necesarios para la interfaz gráfica y manejo de imágenes
import tkinter as tk
from tkinter import ttk
from tkinter import messagebox, StringVar
from PIL import Image, ImageTk  # Para manejar imágenes dentro de Tkinter

# Función para mostrar la ventana de presentación
def Presentacion():
    """
    Esta función cocnsite en crear una ventana con la presentación del proyecto. 
    """
    # Crear una nueva ventana independiente usando Toplevel
    ventana_presentacion = tk.Toplevel()  
    ventana_presentacion.title("Presentación del Proyecto")  # Título de la ventana

    # Crear el texto de presentación utilizando un formato para alinear el contenido
    texto_presentacion = (
        "\n{:^70}\n"  # Universidad
        "{:^70}\n"  # Facultad
        "{:^70}\n\n"  # Departamento
    ...
    # Crear un Label para mostrar el texto de presentación
    label = tk.Label(
        ventana_presentacion,  # La ventana en la que se colocará el Label
        text=texto_presentacion,  # Texto a mostrar
        justify="center",  # Centrar el texto
        padx=10,  # Espaciado horizontal interno
        pady=10   # Espaciado vertical interno
    )
    label.pack()  # Empaquetar el Label dentro de la ventana

    # Botón para cerrar la ventana de presentación y regresar a la ventana principal
    exit_button = tk.Button(
        ventana_presentacion,  # Ventana en la que se colocará el botón
        text="REGRESAR",  # Texto del botón
        command=lambda: ventana_presentacion.destroy()  # Acción al hacer clic: cerrar la ventana
    )
    exit_button.pack(pady=20)  # Empaquetar el botón con un margen vertical
...
def ProvinciasComarcas():
    ...
    ventana_provincias.title("Provincias")  # Título de la ventana
    ventana_provincias.geometry("600x450")  # Dimensiones de la ventana

    # Crear un marco para organizar los elementos dentro de la ventana
    frame = tk.Frame(ventana_provincias)
    frame.pack(pady=10)  # Añadir un margen vertical dentro del frame

    try:
        # Intentar cargar una imagen desde la ruta especificada
        image_path = "imagenes/empresas_hou.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Abrir la imagen usando PIL
        image = image.resize((500, 400), Image.LANCZOS)  # Redimensionar la imagen a un tamaño específico
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para usarla en Tkinter

        # Crear un widget Label para mostrar la imagen
        image_label = tk.Label(frame, image=photo)
        image_label.image = photo  # Asignar la referencia de la imagen para evitar que sea recolectada por el garbage collector
        image_label.pack(pady=10)  # Añadir un margen vertical para el Label
    except Exception as e:
        # Manejar cualquier error en la carga de la imagen mostrando un mensaje de error
        messagebox.showerror("Error", f"No se pudo cargar la imagen: {e}")

    # Crear la barra de menú para la ventana
    BarraProvincias = tk.Menu(ventana_provincias)
    ventana_provincias.config(menu=BarraProvincias)  # Asociar la barra de menú a la ventana

    # Crear el menú desplegable para Provincias
    menu_provincias = tk.Menu(BarraProvincias, tearoff=0)  # Crear un menú sin línea separadora
    menu_provincias.add_command(label="Bocas del Toro", command=Bocasdeltoro)  # Opción para Bocas del Toro
    menu_provincias.add_command(label="Coclé", command=Cocle)  # Opción para Coclé
    menu_provincias.add_command(label="Colón", command=Colon)  # Opción para Colón
    menu_provincias.add_command(label="Chiriquí", command=Chiriqui)  # Opción para Chiriquí
    menu_provincias.add_command(label="Darién", command=Darien)  # Opción para Darién
    menu_provincias.add_command(label="Herrera", command=Herrera)  # Opción para Herrera
    menu_provincias.add_command(label="Los Santos", command=LosSantos)  # Opción para Los Santos
    menu_provincias.add_command(label="Panamá", command=Panama)  # Opción para Panamá
    menu_provincias.add_command(label="Veraguas", command=Veraguas)  # Opción para Veraguas
    menu_provincias.add_command(label="Panamá Oeste", command=PanamaOeste)  # Opción para Panamá Oeste

    # Añadir el menú de Provincias a la barra de menú principal
    BarraProvincias.add_cascade(label="Provincias ", menu=menu_provincias)

    # Crear el menú desplegable para Comarcas
    menu_comarcas = tk.Menu(BarraProvincias, tearoff=0)  # Crear un menú sin línea separadora
    menu_comarcas.add_command(label="Embera-Wounaan", command=EmberaWounaan)  # Opción para Embera-Wounaan
    menu_comarcas.add_command(label="Guna Yala", command=GunaYala)  # Opción para Guna Yala
    menu_comarcas.add_command(label="Ngabe-Bugle", command=NgabeBugle)  # Opción para Ngabe-Bugle

    # Añadir el menú de Comarcas a la barra de menú principal
    BarraProvincias.add_cascade(label="Comarcas", menu=menu_comarcas)

    # Crear un botón para cerrar la ventana
    exit_button = tk.Button(
        ventana_provincias,  # Ventana en la que se colocará el botón
        text="REGRESAR",  # Texto que aparecerá en el botón
        command=ventana_provincias.destroy  # Acción al hacer clic: cerrar la ventana
    )
    exit_button.pack(pady=20)  # Añadir un margen vertical alrededor del botón


def Bocasdeltoro():
    # Crear una nueva ventana secundaria para Bocas del Toro
    ventana_bocas = tk.Toplevel()
    ventana_bocas.title("Bocas del Toro")  # Título de la ventana
    ventana_bocas.geometry("600x400")  # Dimensiones de la ventana

    # Añadir un título principal con el nombre de la provincia
    title_label = tk.Label(
        ventana_bocas,
        text="Provincia: Bocas del Toro",
        font=("Arial", 16, "bold")  # Fuente, tamaño y estilo del texto
    )
    title_label.pack(pady=10)  # Añadir un margen vertical

    # Crear un marco para organizar los elementos dentro de la ventana
    frame = tk.Frame(ventana_bocas)
    frame.pack(pady=10)  # Margen vertical para separar el frame del título
    ...
    # Crear un Label para mostrar la descripción con formato de texto ajustado
    description_label = tk.Label(
        frame,
        text=descripcionBocas,
        wraplength=500,  # Ajustar el texto para que no exceda este ancho
        justify="left"  # Alinear el texto a la izquierda
    )
    description_label.pack()  # Colocar el Label en el frame

    # Cargar y mostrar una imagen representativa de la provincia
    try:
        image_path = "imagenes/Bocasdeltoro/bocasdeltoro.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Abrir la imagen usando PIL
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(frame, image=photo)  # Crear un Label para mostrar la imagen
        image_label.image = photo  # Mantener una referencia a la imagen para evitar que sea recolectada
        image_label.pack(pady=10)  # Añadir un margen vertical
    except Exception as e:
        # Manejar cualquier error al cargar la imagen mostrando un mensaje al usuario
        messagebox.showerror("Error", f"No se pudo cargar la imagen: {e}")

    # Crear un menú desplegable para mostrar lugares turísticos de la provincia
    lugaresBocas = tk.Menu(ventana_bocas)  # Crear la barra de menú
    ventana_bocas.config(menu=lugaresBocas)  # Asociar la barra de menú a la ventana

    # Crear el submenú con opciones de lugares turísticos
    menu_bocas = tk.Menu(lugaresBocas, tearoff=0)  # Menú sin línea separadora
    menu_bocas.add_command(label="Playa Estrella", command=PlayaEstrellaBocas)  # Opción para Playa Estrella
    menu_bocas.add_command(label="Green Acres", command=GreenAcresBocas)  # Opción para Green Acres
    menu_bocas.add_command(label="Bahía de los Delfines", command=BahiaDelfinesBocas)  # Opción para Bahía de los Delfines
    menu_bocas.add_command(label="Aldea de Botellas Plásticas", command=AldeaBotellasBocas)  # Opción para la Aldea de Botellas Plásticas

    # Añadir el submenú a la barra de menú principal
    lugaresBocas.add_cascade(
        label="Lugares Turistico de Bocas del Toro que deberias visitar",
        menu=menu_bocas
    )

    # Crear un botón para regresar a la ventana anterior
    boton_regresar = tk.Button(
        ...
        command=ventana_bocas.destroy  # Acción al hacer clic: cerrar la ventana actual
    )
    # Posicionar el botón en la esquina inferior derecha
    boton_regresar.place(x=450, y=335)

def PlayaEstrellaBocas():
    # Crear una nueva ventana secundaria para la actividad "Playa Estrella"
    ventana_PlayaEstrella = tk.Toplevel()
    ventana_PlayaEstrella.title("Playa Estrella")  # Título de la ventana
    ventana_PlayaEstrella.geometry("700x600")  # Dimensiones de la ventana
    ventana_PlayaEstrella.configure(bg='#009999')  # Color de fondo de la ventana

    # Frame principal para organizar todos los elementos dentro de la ventana
    main_frame = tk.Frame(ventana_PlayaEstrella, bg='#009999')  # Fondo del frame igual al de la ventana
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Margen interior y ajuste completo del espacio disponible

    # Título de la actividad dentro del frame principal
    titulo = tk.Label(main_frame, text="Playa Estrella", font=("Arial", 20, "bold"), bg='#009999')  # Fuente y estilo del texto
    titulo.pack(pady=10)  # Margen superior e inferior

    # Intentar cargar y mostrar la imagen de la actividad
    try:
        ...
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen con un método de alta calidad
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen a un formato compatible con Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#009999')  # Label para mostrar la imagen
        image_label.image = photo  # Mantener referencia a la imagen
        image_label.pack(pady=10)  # Margen superior e inferior
    except Exception as e:
        # Manejar errores si no se puede cargar la imagen
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Frame para la descripción de la actividad
    descripcion_frame = tk.Frame(
        main_frame,
        bg="white",  # Fondo blanco para contraste
        bd=2,  # Bordes del frame
        relief="solid",  # Estilo de borde
        padx=15,  # Margen interno horizontal
        pady=15,  # Margen interno vertical
        width=300  # Ancho inicial
    )
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Posicionar a la izquierda y ajustar en altura
    ...
    descripcion = tk.Label(
        ...
        wraplength=280,  # Ajustar el texto para no exceder este ancho
        justify="left",  # Alinear el texto a la izquierda
        bg="white",  # Fondo blanco para coincidir con el frame
        font=("Arial", 10)  # Tamaño de fuente pequeño y claro
    )
    descripcion.pack()  # Posicionar dentro del frame de descripción

    # Frame para mostrar los detalles de la actividad
    detalles_frame = tk.Frame(
        main_frame,
    ...
    detalles_frame.pack(pady=15, side="right", fill="y")  # Posicionar a la derecha y ajustar en altura

    # Texto con detalles adicionales de la actividad
    detalles_texto = (
        "Incluye:\n"
    ...
    # Label para mostrar los detalles
    detalles = tk.Label(
        detalles_frame,
        text=detalles_texto,
        wraplength=280,  # Ajustar el texto para no exceder este ancho
    ...
    detalles.pack()  # Posicionar dentro del frame de detalles

    # Frame para los botones de acción
    botones_frame = tk.Frame(ventana_PlayaEstrella, bg='#009999')  # Fondo igual al de la ventana principal
    botones_frame.pack(pady=20)  # Separar del contenido anterior

    # Botón "RESERVAR"
    boton_reservar = tk.Button(
        botones_frame,
        text="RESERVAR",  # Texto del botón
        width=15,  # Ancho del botón
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=lambda: Reserva(nombre_actividad="Playa Estrella")  # Llamar a la función de reserva con un parámetro
    )
    boton_reservar.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal
    ...
    boton_volver = tk.Button(
        ...
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=ventana_PlayaEstrella.destroy  # Cerrar la ventana actual al hacer clic
    )
    boton_volver.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal


def GreenAcresBocas():
    # Crear una nueva ventana secundaria para la actividad "Green Acres"
    ventana_GreenAcres = tk.Toplevel()
    ventana_GreenAcres.title("Green Acres")  # Título de la ventana
    ventana_GreenAcres.geometry("700x600")  # Dimensiones de la ventana
    ventana_GreenAcres.configure(bg='#009999')  # Color de fondo turquesa de la ventana

    # Frame principal para organizar todos los elementos dentro de la ventana
    main_frame = tk.Frame(ventana_GreenAcres, bg='#009999')  # Fondo igual al de la ventana
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Margen interior y ajuste completo del espacio disponible

    # Título de la actividad dentro del frame principal
    titulo = tk.Label(main_frame, text="GREEN ACRES", font=("Arial", 20, "bold"), bg='#009999')  # Fuente y estilo del texto
    titulo.pack(pady=10)  # Margen superior e inferior para el título
    ...
    descripcion_frame = tk.Frame(
        ...
        padx=15,  # Margen interno horizontal
        pady=15,  # Margen interno vertical
        width=300  # Ancho inicial del frame
    )
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Posicionar a la izquierda y ajustar en altura
    ...
    descripcion = tk.Label(
        ...
        justify="left",  # Alinear el texto a la izquierda
        bg="white",  # Fondo blanco para coincidir con el frame
        font=("Arial", 10)  # Fuente y tamaño de texto pequeño
    )
    descripcion.pack()  # Posicionar dentro del frame de descripción
    ...
    detalles_frame = tk.Frame(
        main_frame,
        bg="white",  # Fondo blanco para el contraste
        bd=2,  # Bordes del frame
        relief="solid",  # Estilo de borde
        padx=15,  # Margen interno horizontal
        pady=15,  # Margen interno vertical
    ...
    # Frame para los botones de acción (Reserva y Volver)
    botones_frame = tk.Frame(ventana_GreenAcres, bg='#009999')  # Fondo igual al de la ventana
    botones_frame.pack(pady=20)  # Separar de los contenidos anteriores

    # Botón "RESERVAR"
    boton_reservar = tk.Button(
        botones_frame,
        text="RESERVAR",  # Texto que aparece en el botón
        width=15,  # Ancho del botón
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=lambda: Reserva(nombre_actividad="GREEN ACRES")  # Llamar a la función de reserva con el nombre de la actividad
    )
    boton_reservar.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal
    ...
    boton_volver = tk.Button(
        botones_frame,
        text="VOLVER",  # Texto que aparece en el botón
        width=15,  # Ancho del botón
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=ventana_GreenAcres.destroy  # Cerrar la ventana actual al hacer clic
    )
    boton_volver.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal
 
def BahiaDelfinesBocas():
    # Crear una nueva ventana secundaria para la actividad "Bahía de los Delfines"
    ventana_BahiaDelfines = tk.Toplevel()
    ventana_BahiaDelfines.title("Bahía de los Delfines")  # Título de la ventana
    ventana_BahiaDelfines.geometry("700x600")  # Dimensiones de la ventana
    ventana_BahiaDelfines.configure(bg='#009999')  # Fondo color turquesa de la ventana

    # Frame principal para organizar todos los elementos dentro de la ventana
    main_frame = tk.Frame(ventana_BahiaDelfines, bg='#009999')  # Fondo igual al de la ventana
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Margen interior y ajuste completo del espacio disponible

    # Título de la actividad dentro del frame principal
    titulo = tk.Label(main_frame, text="BAHÍA DE LOS DELFINES", font=("Arial", 20, "bold"), bg='#009999')  # Fuente y estilo del texto
    titulo.pack(pady=10)  # Margen superior e inferior para el título
    ...
    # Frame para los botones de acción (Reserva y Volver)
    botones_frame = tk.Frame(ventana_BahiaDelfines, bg='#009999')  # Fondo igual al de la ventana
    botones_frame.pack(pady=20)  # Separar de los contenidos anteriores
    ...
    boton_reservar = tk.Button(
        ...
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=lambda: Reserva(nombre_actividad="Bahía de los Delfines")  # Llamar a la función de reserva con el nombre de la actividad
    )
    boton_reservar.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal
    ...
    boton_volver = tk.Button(
        ...
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=ventana_BahiaDelfines.destroy  # Cerrar la ventana actual al hacer clic
    )
    boton_volver.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal

def AldeaBotellasBocas():
    # Crear una nueva ventana secundaria para la actividad "Aldea de Botellas Plásticas"
    ventana_AldeaBotellas = tk.Toplevel()
    ventana_AldeaBotellas.title("Aldea de Botellas Plásticas")  # Título de la ventana
    ventana_AldeaBotellas.geometry("700x600")  # Dimensiones de la ventana
    ventana_AldeaBotellas.configure(bg='#009999')  # Fondo color turquesa de la ventana

    # Frame principal para organizar todos los elementos dentro de la ventana
    main_frame = tk.Frame(ventana_AldeaBotellas, bg='#009999')  # Fondo igual al de la ventana
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Margen interior y ajuste completo del espacio disponible

    # Título de la actividad dentro del frame principal
    titulo = tk.Label(main_frame, text="ALDEA DE BOTELLAS PLÁSTICAS", font=("Arial", 20, "bold"), bg='#009999')  # Fuente y estilo del texto
    titulo.pack(pady=10)  # Margen superior e inferior para el título
    ...
    # Frame para los botones de acción (Reserva y Volver)
    botones_frame = tk.Frame(ventana_AldeaBotellas, bg='#009999')  # Fondo igual al de la ventana
    botones_frame.pack(pady=20)  # Separar de los contenidos anteriores
    ...
    boton_reservar = tk.Button(
        ...
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=lambda: Reserva(nombre_actividad="Aldea de Botellas Plásticas")  # Llamar a la función de reserva con el nombre de la actividad
    )
    boton_reservar.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal
    ...
    boton_volver = tk.Button(
        ...
        font=("Arial", 12),  # Fuente y tamaño del texto
        bg="white",  # Fondo blanco para contraste
        command=ventana_AldeaBotellas.destroy  # Cerrar la ventana actual al hacer clic
    )
    boton_volver.pack(side="left", padx=10)  # Posicionar a la izquierda con margen horizontal


def Cocle():
    # Crear una nueva ventana secundaria para mostrar información sobre la provincia de Coclé
    ventana_cocle = tk.Toplevel()  # Crear la ventana secundaria
    ventana_cocle.title("Cocle")  # Título de la ventana
    ventana_cocle.geometry("600x400")  # Dimensiones de la ventana

    # Título de la provincia dentro de la ventana
    title_label = tk.Label(ventana_cocle, text="Provincia: Cocle", font=("Arial", 16, "bold"))  # Estilo del título
    title_label.pack(pady=10)  # Añadir el título con margen superior

    # Crear un frame para organizar los contenidos dentro de la ventana
    frame = tk.Frame(ventana_cocle)  # Frame donde se colocarán otros elementos
    frame.pack(pady=10)  # Añadir el frame con margen superior

    # Descripción de la provincia de Coclé
    descripcionCocle = (
    ...
    # Etiqueta para mostrar la descripción dentro del frame
    description_label = tk.Label(frame, text=descripcionCocle, wraplength=500, justify="left")  # Ajuste de texto y alineación
    description_label.pack()  # Añadir la etiqueta al frame

    # Intentar cargar y mostrar la imagen de la provincia de Coclé
    try:
        image_path = "imagenes/Cocle/Cocle.jpg"  # Ruta a la imagen
        image = Image.open(image_path)  # Cargar la imagen con PIL
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen para ajustarla al espacio
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen a un formato compatible con Tkinter
        image_label = tk.Label(frame, image=photo)  # Etiqueta para mostrar la imagen
        image_label.image = photo  # Mantener referencia a la imagen
        image_label.pack(pady=10)  # Añadir la imagen con margen superior e inferior
    except Exception as e:
        # Manejar errores si no se puede cargar la imagen
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Crear un menú de opciones para seleccionar lugares turísticos dentro de la provincia de Coclé
    lugaresCocle = tk.Menu(ventana_cocle)  # Crear el menú principal de la ventana
    ventana_cocle.config(menu=lugaresCocle)  # Asociar el menú a la ventana

    # Crear un submenú dentro del menú principal
    menu_cocle = tk.Menu(lugaresCocle, tearoff=0)  # Crear el submenú sin separación visual
    menu_cocle.add_command(label="Zoológico El Níspero", command= ZoologicoElNispero)  # Añadir opción para el zoológico
    menu_cocle.add_command(label="Mariposario del Valle", command= MariposarioDelValle)  # Añadir opción para el mariposario
    menu_cocle.add_command(label="Sendero de La India Dormida", command= SenderoIndiaDormidaCocle)  # Añadir opción para el sendero
    menu_cocle.add_command(label="Serpentario Maravillas Tropicales", command= SerpentarioMaravillasCocle)  # Añadir opción para el serpentario

    # Añadir el submenú al menú principal
    lugaresCocle.add_cascade(label="Lugares Turísticos de Coclé que deberías visitar", menu=menu_cocle)

    # Botón para regresar a la ventana anterior o cerrar la ventana actual
    boton_regresar = tk.Button(ventana_cocle, text="Regresar", width=15, font=("Arial", 12), bg="white", command=ventana_cocle.destroy)
    boton_regresar.place(x=450, y=335)  # Colocar el botón en una posición específica dentro de la ventana

#Inicio Lugares Turisticos de Cocle
def ZoologicoElNispero():
    # Crear la ventana secundaria para el zoológico El Níspero
    ventana_ZoologicoElNispero = tk.Toplevel()  # Crear la ventana secundaria
    ventana_ZoologicoElNispero.title("Zoológico El Níspero")  # Título de la ventana
    ventana_ZoologicoElNispero.geometry("700x600")  # Dimensiones de la ventana
    ventana_ZoologicoElNispero.configure(bg='#D3D3D3')  # Configuración de color de fondo de la ventana

    # Frame principal para organizar todos los elementos de la ventana
    main_frame = tk.Frame(ventana_ZoologicoElNispero, bg='#D3D3D3')  # Frame principal
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de la actividad en el zoológico
    titulo = tk.Label(main_frame, text="Zoológico El Níspero", font=("Arial", 20, "bold"), bg='#D3D3D3')  # Etiqueta de título
    titulo.pack(pady=10)  # Añadir la etiqueta con margen superior

    # Intentar cargar y mostrar la imagen del zoológico
    try:
        image_path = "imagenes/Cocle/ZoologicoElNispero.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen al formato de Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#D3D3D3')  # Mostrar la imagen en una etiqueta
        image_label.image = photo  # Mantener la referencia de la imagen
        image_label.pack(pady=10)  # Empaquetar la imagen con margen
    except Exception as e:
        # Manejar errores si la imagen no se carga correctamente
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Frame para la descripción del zoológico
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame de descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame de descripción

    # Texto de la descripción del zoológico
    descripcion_texto = (
    ...
    # Etiqueta que muestra la descripción del zoológico
    descripcion = tk.Label(descripcion_frame, text=descripcion_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    descripcion.pack()  # Empaquetar la descripción

    # Frame para los detalles de la actividad
    detalles_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para los detalles
    detalles_frame.pack(pady=15, side="right", fill="y")  # Empaquetar el frame de detalles

    # Texto con los detalles de la actividad
    detalles_texto = (
        "Incluye:\n"
    ...
    # Etiqueta que muestra los detalles de la actividad
    detalles = tk.Label(detalles_frame, text=detalles_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    detalles.pack()  # Empaquetar los detalles

    # Frame para los botones de acción
    botones_frame = tk.Frame(ventana_ZoologicoElNispero, bg='#D3D3D3')  # Frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame con margen inferior
    ...
    boton_reservar.pack(side="left", padx=10)  # Empaquetar el botón de reserva a la izquierda
    ...
    boton_volver.pack(side="left", padx=10)  # Empaquetar el botón de volver a la izquierda
  
def MariposarioDelValle():
    # Crear la ventana secundaria para el Mariposario del Valle
    ventana_MariposarioDelValle = tk.Toplevel()  # Crear la ventana secundaria
    ventana_MariposarioDelValle.title("Mariposario del Valle")  # Título de la ventana
    ventana_MariposarioDelValle.geometry("700x600")  # Dimensiones de la ventana
    ventana_MariposarioDelValle.configure(bg='#D3D3D3')  # Configuración de color de fondo de la ventana

    # Frame principal para organizar todos los elementos de la ventana
    main_frame = tk.Frame(ventana_MariposarioDelValle, bg='#D3D3D3')  # Frame principal
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de la actividad en el mariposario
    titulo = tk.Label(main_frame, text="Mariposario del Valle", font=("Arial", 20, "bold"), bg='#D3D3D3')  # Etiqueta de título
    titulo.pack(pady=10)  # Añadir la etiqueta con margen superior

    # Intentar cargar y mostrar la imagen del mariposario
    try:
        image_path = "imagenes/Cocle/MariposarioDelValle.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen al formato de Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#D3D3D3')  # Mostrar la imagen en una etiqueta
    ...
    # Frame para la descripción del mariposario
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame de descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame de descripción

    # Texto de la descripción del mariposario
    descripcion_texto = (
    ...
    # Etiqueta que muestra la descripción del mariposario
    descripcion = tk.Label(descripcion_frame, text=descripcion_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    descripcion.pack()  # Empaquetar la descripción
    ...
    # Frame para los botones de acción
    botones_frame = tk.Frame(ventana_MariposarioDelValle, bg='#D3D3D3')  # Frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame con margen inferior
...
def SenderoIndiaDormidaCocle():
    # Crear la ventana secundaria para el Sendero de La India Dormida
    ventana_IndiaDormida = tk.Toplevel()  # Crear una nueva ventana secundaria
    ventana_IndiaDormida.title("Sendero de La India Dormida")  # Título de la ventana
    ventana_IndiaDormida.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana_IndiaDormida.configure(bg='#D3D3D3')  # Establecer color de fondo de la ventana

    # Frame principal donde se colocan todos los elementos de la ventana
    main_frame = tk.Frame(ventana_IndiaDormida, bg='#D3D3D3')  # Frame principal
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de la actividad del sendero
    titulo = tk.Label(main_frame, text="Sendero de La India Dormida", font=("Arial", 20, "bold"), bg='#D3D3D3')  # Etiqueta con el título
    titulo.pack(pady=10)  # Empaquetar el título con margen superior

    # Intentar cargar y mostrar la imagen del sendero
    try:
        image_path = "imagenes/Cocle/SenderoIndiaDormida.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#D3D3D3')  # Crear etiqueta para mostrar la imagen
        image_label.image = photo  # Mantener la referencia de la imagen
        image_label.pack(pady=10)  # Empaquetar la imagen con margen inferior
    except Exception as e:
        # Manejar errores si la imagen no se carga correctamente
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Frame para la descripción del sendero
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para la descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame con márgenes y en el lado izquierdo

    # Texto que describe el sendero
    descripcion_texto = (
    ...
    # Etiqueta que muestra la descripción del sendero
    descripcion = tk.Label(descripcion_frame, text=descripcion_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    descripcion.pack()  # Empaquetar la descripción dentro del frame

    # Frame para los detalles de la actividad
    detalles_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para los detalles
    detalles_frame.pack(pady=15, side="right", fill="y")  # Empaquetar el frame de detalles en el lado derecho
    ...
    # Etiqueta para mostrar los detalles de la actividad
    detalles = tk.Label(detalles_frame, text=detalles_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    detalles.pack()  # Empaquetar los detalles dentro del frame

    # Frame para los botones de acción
    botones_frame = tk.Frame(ventana_IndiaDormida, bg='#D3D3D3')  # Frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame de botones con márgenes inferiores
...
def SerpentarioMaravillasCocle():
    # Crear la ventana secundaria para el Serpentario Maravillas Tropicales
    ventana_Serpentario = tk.Toplevel()  # Crear una nueva ventana secundaria
    ventana_Serpentario.title("Serpentario Maravillas Tropicales")  # Título de la ventana
    ventana_Serpentario.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana_Serpentario.configure(bg='#D3D3D3')  # Establecer el color de fondo de la ventana

    # Frame principal donde se colocan todos los elementos de la ventana
    main_frame = tk.Frame(ventana_Serpentario, bg='#D3D3D3')  # Crear un frame principal para los elementos
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de la actividad del serpentario
    titulo = tk.Label(main_frame, text="Serpentario Maravillas Tropicales", font=("Arial", 20, "bold"), bg='#D3D3D3')  # Etiqueta con el título
    titulo.pack(pady=10)  # Empaquetar el título con un margen superior

    # Intentar cargar y mostrar la imagen del serpentario
    try:
        image_path = "imagenes/Cocle/Serpentario.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#D3D3D3')  # Crear una etiqueta para mostrar la imagen
        image_label.image = photo  # Mantener la referencia de la imagen
        image_label.pack(pady=10)  # Empaquetar la imagen con un margen inferior
    except Exception as e:
        # Manejar errores si la imagen no se carga correctamente
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Frame para la descripción del serpentario
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para la descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame en el lado izquierdo con un margen

    # Texto que describe el serpentario
    descripcion_texto = (
        "Este serpentario en el Valle de Antón alberga una variedad de reptiles y ofrece una experiencia educativa "
        "sobre la importancia de la conservación."
    )
    # Etiqueta que muestra la descripción del serpentario
    descripcion = tk.Label(descripcion_frame, text=descripcion_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))
    descripcion.pack()  # Empaquetar la descripción dentro del frame
    ...
    # Frame para los botones de acción
    botones_frame = tk.Frame(ventana_Serpentario, bg='#D3D3D3')  # Frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame de botones con márgenes inferiores
...
def Colon():
    # Crear una nueva ventana secundaria para mostrar la información de Colón
    ventana_colon = tk.Toplevel()  # Crear la ventana secundaria
    ventana_colon.title("Colon")  # Establecer el título de la ventana
    ventana_colon.geometry("600x400")  # Definir las dimensiones de la ventana

    # Etiqueta para el título de la provincia
    title_label = tk.Label(ventana_colon, text="Provincia: Colon", font=("Arial", 16, "bold"))  # Título en la ventana
    title_label.pack(pady=10)  # Empaquetar la etiqueta con un margen superior

    # Frame para contener la descripción y la imagen
    frame = tk.Frame(ventana_colon)  # Crear un frame para la descripción e imagen
    frame.pack(pady=10)  # Empaquetar el frame con un margen inferior

    # Descripción de la provincia de Colón
    descripcionColon = (
    ...
    # Etiqueta para mostrar la descripción de Colón
    description_label = tk.Label(frame, text=descripcionColon, wraplength=500, justify="left")  # Descripción en una etiqueta
    description_label.pack()  # Empaquetar la descripción dentro del frame

    # Intentar cargar y mostrar la imagen de Colón
    try:
        image_path = "imagenes/Colon/Colon.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
    ...
    # Crear el menú de lugares turísticos de Colón
    lugaresColon = tk.Menu(ventana_colon)  # Menú principal de lugares turísticos
    ventana_colon.config(menu=lugaresColon)  # Configurar el menú en la ventana

    # Crear el submenú con los lugares turísticos
    menu_colon = tk.Menu(lugaresColon, tearoff=0)  # Menú para los lugares turísticos
    menu_colon.add_command(label="El Castillo de San Lorenzo El Real de Chagres", command=CastilloSanLorenzo)  # Opción para el Castillo
    menu_colon.add_command(label="Esclusas de Gatún", command=EsclusasGatun)  # Opción para las Esclusas de Gatún
    menu_colon.add_command(label="Esclusas de agua clara", command=EsclusasAguaClaraColon)  # Opción para las Esclusas de agua clara
    menu_colon.add_command(label="Parque Nacional Chagres", command=ParqueNacionalChagres)  # Opción para el Parque Nacional Chagres
    
    # Añadir el submenú al menú principal
    lugaresColon.add_cascade(label="Lugares Turísticos de Colón que deberías visitar", menu=menu_colon)

    # Botón para regresar a la ventana anterior
    boton_regresar = tk.Button(ventana_colon, text="Regresar", width=15, font=("Arial", 12), bg="white", command=ventana_colon.destroy)
    boton_regresar.place(x=450, y=335)  # Colocar el botón de regresar en la parte inferior derecha

#Inicio Lugares Turisticos de Colon   
def CastilloSanLorenzo():
    # Crear una nueva ventana para el Castillo de San Lorenzo
    ventana_CastilloSanLorenzo = tk.Toplevel()  # Crear ventana secundaria
    ventana_CastilloSanLorenzo.title("Castillo de San Lorenzo")  # Establecer el título de la ventana
    ventana_CastilloSanLorenzo.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana_CastilloSanLorenzo.configure(bg='#2F4F4F')  # Configurar el color de fondo de la ventana

    # Frame principal donde se colocarán los elementos
    main_frame = tk.Frame(ventana_CastilloSanLorenzo, bg='#2F4F4F')  # Frame con color de fondo
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título del castillo
    titulo = tk.Label(main_frame, text="Castillo de San Lorenzo", font=("Arial", 20, "bold"), bg='#2F4F4F')  # Título con estilo
    titulo.pack(pady=10)  # Empaquetar el título con un margen

    # Intentar cargar y mostrar la imagen del Castillo de San Lorenzo
    try:
        image_path = "imagenes/Colon/CastilloSanLorenzo.jpg"  # Ruta a la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#2F4F4F')  # Crear etiqueta para la imagen
        image_label.image = photo  # Mantener la referencia de la imagen
        image_label.pack(pady=10)  # Empaquetar la imagen con un margen inferior
    except Exception as e:
        # Si no se puede cargar la imagen, mostrar un mensaje de error
        messagebox.showerror("Error", f"Error, no se pudo cargar la imagen: {e}")

    # Descripción del Castillo de San Lorenzo
    descripcion_texto = (
    ...
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para la descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame
    descripcion = tk.Label(descripcion_frame, text=descripcion_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))  # Etiqueta con la descripción
    descripcion.pack()  # Empaquetar la descripción

    # Detalles adicionales sobre la actividad en el castillo
    detalles_texto = (
        "Incluye:\n"
    ...
    detalles_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para los detalles
    detalles_frame.pack(pady=15, side="right", fill="y")  # Empaquetar el frame
    detalles = tk.Label(detalles_frame, text=detalles_texto, wraplength=280, justify="left", bg="white", font=("Arial", 10))  # Etiqueta con los detalles
    detalles.pack()  # Empaquetar los detalles

    # Botón para reservar la actividad en el Castillo de San Lorenzo
    ...
    boton_reservar.pack(side="left", padx=10, pady=20)  # Empaquetar el botón con márgenes
    ...
    boton_volver.pack(side="left", padx=10)  # Empaquetar el botón de volver

def EsclusasGatun():
    # Crear una nueva ventana para las Esclusas de Gatún
    ventana_EsclusasGatun = tk.Toplevel()  # Crear ventana secundaria
    ventana_EsclusasGatun.title("Esclusas de Gatún")  # Establecer el título de la ventana
    ventana_EsclusasGatun.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana_EsclusasGatun.configure(bg='#2F4F4F')  # Configurar el color de fondo de la ventana

    # Frame principal donde se colocarán los elementos
    main_frame = tk.Frame(ventana_EsclusasGatun, bg='#2F4F4F')  # Frame con color de fondo
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de las esclusas
    titulo = tk.Label(main_frame, text="Esclusas de Gatún", font=("Arial", 20, "bold"), bg='#2F4F4F')  # Título con estilo
    titulo.pack(pady=10)  # Empaquetar el título con un margen

    # Intentar cargar y mostrar la imagen de las Esclusas de Gatún
    try:
        image_path = "imagenes/Colon/EsclusasGatun.jpg"  # Ruta a la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#2F4F4F')  # Crear etiqueta para la imagen
    ...
    # Descripción de las Esclusas de Gatún
    descripcion_texto = (
    ...
    # Detalles adicionales sobre la actividad en las esclusas
    detalles_texto = (
        "Incluye:\n"
    ...
    # Botón para reservar la actividad en las Esclusas de Gatún
...
def EsclusasAguaClaraColon():
    # Crear una nueva ventana para las Esclusas de Agua Clara
    ventana_AguaClara = tk.Toplevel()  # Crear ventana secundaria
    ventana_AguaClara.title("Esclusas de Agua Clara")  # Establecer el título de la ventana
    ventana_AguaClara.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana_AguaClara.configure(bg='#2F4F4F')  # Configurar el color de fondo de la ventana

    # Frame principal donde se colocarán los elementos
    main_frame = tk.Frame(ventana_AguaClara, bg='#2F4F4F')  # Frame con color de fondo
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de las Esclusas de Agua Clara
    titulo = tk.Label(main_frame, text="Esclusas de Agua Clara", font=("Arial", 20, "bold"), bg='#2F4F4F')  # Título con estilo
    titulo.pack(pady=10)  # Empaquetar el título con un margen

    # Intentar cargar y mostrar la imagen de las Esclusas de Agua Clara
    try:
        image_path = "imagenes/Colon/EsclusasAguaClara.jpg"  # Ruta a la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#2F4F4F')  # Crear etiqueta para la imagen
    ...
    # Descripción de las Esclusas de Agua Clara
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para la descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame
    ...
    # Frame para los botones (reservar y volver)
    botones_frame = tk.Frame(ventana_AguaClara, bg='#2F4F4F')  # Crear frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame de los botones

    # Botón para reservar la actividad en las Esclusas de Agua Clara
    ...
    boton_reservar.pack(side="left", padx=10)  # Empaquetar el botón de reservar
...
def ParqueNacionalChagres():
    # Crear una ventana para mostrar información sobre el Parque Nacional Chagres
    ventana = tk.Toplevel()  # Crear una nueva ventana secundaria
    ventana.title("Parque Nacional Chagres")  # Establecer el título de la ventana
    ventana.geometry("700x600")  # Definir las dimensiones de la ventana
    ventana.configure(bg='#2F4F4F')  # Establecer color de fondo

    # Frame principal donde se organizarán los elementos
    main_frame = tk.Frame(ventana, bg='#2F4F4F')  # Crear frame con color de fondo
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)  # Empaquetar el frame con márgenes

    # Título de la actividad "Parque Nacional Chagres"
    titulo = tk.Label(main_frame, text="Parque Nacional Chagres", font=("Arial", 20, "bold"), bg='#2F4F4F')  # Crear etiqueta con el título
    titulo.pack(pady=10)  # Empaquetar la etiqueta con un margen

    # Intentar cargar y mostrar la imagen del Parque Nacional Chagres
    try:
        image_path = "imagenes/Colon/ParqueNacionalChagres.jpg"  # Ruta de la imagen
        image = Image.open(image_path)  # Cargar la imagen
        image = image.resize((250, 150), Image.LANCZOS)  # Redimensionar la imagen
        photo = ImageTk.PhotoImage(image)  # Convertir la imagen para Tkinter
        image_label = tk.Label(main_frame, image=photo, bg='#2F4F4F')  # Etiqueta con la imagen
        image_label.image = photo  # Mantener la referencia de la imagen
        image_label.pack(pady=10)  # Empaquetar la imagen con un margen inferior
    except Exception as e:
        # Si ocurre un error al cargar la imagen, mostrar mensaje de error
        messagebox.showerror("Error", f"Could not load image: {e}")

    # Descripción del Parque Nacional Chagres
    descripcion_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para la descripción
    descripcion_frame.pack(pady=15, side="left", fill="y")  # Empaquetar el frame
    ...
    # Detalles sobre las actividades disponibles en el parque
    detalles_frame = tk.Frame(main_frame, bg="white", bd=2, relief="solid", padx=15, pady=15, width=300)  # Frame para los detalles
    detalles_frame.pack(pady=15, side="right", fill="y")  # Empaquetar el frame
    ...
    # Frame para los botones (reservar y volver)
    botones_frame = tk.Frame(ventana, bg='#2F4F4F')  # Crear frame para los botones
    botones_frame.pack(pady=20)  # Empaquetar el frame de los botones

    # Botón para reservar la actividad en el Parque Nacional Chagres
