...
def CastilloSanLorenzo():
    ...
    boton_reservar = tk.Button(ventana_CastilloSanLorenzo, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
...
def EsclusasGatun():
    ...
    boton_reservar = tk.Button(ventana_EsclusasGatun, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def CascadasPerdidas():
    ...
    boton_reservar = tk.Button(ventana_CascadasPerdidas, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def ParqueNacionalDarien():
    ...
    boton_reservar = tk.Button(ventana_ParqueNacionalDarien, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def YavizaFinPanamericana():
    ...
    boton_reservar = tk.Button(ventana_YavizaFinPanamericana, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def PeseDestileriasRon():
    ...
    boton_reservar = tk.Button(ventana_PeseDestileriasRon, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def MuseoHerreraChitre():
    ...
    boton_reservar = tk.Button(ventana_MuseoHerreraChitre, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def IslaIguana():
    ...
    boton_reservar = tk.Button(ventana_IslaIguana, text="RESERVAR", width=15, font=("Arial", 12), bg="white", command=Reserva)
    boton_reservar.pack(side="left", padx=10, pady=20)
...
def Reserva(nombre_actividad):
    ...
    if precio_por_persona is not None:
        ...
        sexo_var = StringVar()
        edad_var = StringVar()  # Nueva variable para la edad
        ...
        def realizar_pago():
            ...
            identificacion = identificacion_var.get()
            telefono = telefono_var.get()
            nacionalidad = nacionalidad_var.get()
            try:
                cantidad_personas = int(cantidad_personas_var.get())
                edad = int(edad_var.get())  # Obtener la edad
                precio_total = cantidad_personas * precio_por_persona  # Calcular el total
            except ValueError:
                messagebox.showerror("Error", "Por favor, ingrese un número válido para la cantidad de personas y la edad.")
                return

            # Llamar a la función de pagar con los datos necesarios
            pagar(nombre, identificacion, telefono, nacionalidad, detalles_texto, precio_total, cantidad_personas, edad, sexo_var.get())

        # Botones de Pagar y Abono
        tk.Button(ventana_reserva, text="Pagar", command=realizar_pago).pack(pady=10)
        tk.Button(ventana_reserva, text="Abonar", command=lambda: messagebox.showinfo("Abono", "Funcionalidad de abono no implementada.")).pack(pady=10)
...
# Función de pago
def pagar(nombre, identificacion, telefono, nacionalidad, detalles_reserva, precio_total, cantidad_personas, edad, sexo):
...
# Asegúrate de que al llamar a la función `pagar`, se pase también la edad y el sexo
# Por ejemplo, en la función `realizar_pago`:
# pagar(nombre, identificacion, telefono, nacionalidad, detalles_texto, precio_total, cantidad_personas, edad, sexo_var.get())
